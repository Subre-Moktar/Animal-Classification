...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        # Handels video steams
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        # Opens the file and reads in the self sources
        if os.path.isfile(sources):
            ...
            sources = [sources]
        # How many sources you give it
        n = len(sources)
        self.imgs = [None] * n
        self.sources = [clean_str(x) for x in sources]  # clean source names for later
        # Loops through the sources you have given it
        for i, s in enumerate(sources):
            # Start the thread to read frames from the video stream
            print(f'{i + 1}/{n}: {s}... ', end='')
            url = eval(s) if s.isnumeric() else s
            # Handels youtube videos and collects their information
            if 'youtube.com/' in str(url) or 'youtu.be/' in str(url):  # if source is YouTube video
                check_requirements(('pafy', 'youtube_dl'))
                import pafy
                url = pafy.new(url).getbest(preftype="mp4").url
            # allows for frame reading from a video
